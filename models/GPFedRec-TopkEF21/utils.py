...
def initLogging(logFilename):
    """Init for logging
    """
    # Replace colons with underscores in the log filename
...
def adaptive_regularization_user(user_id, user_interaction_count, avg_interaction_count, reg_min, reg_max):
    """用户自适应正则化：根据用户交互数量调整正则化系数"""
    # 交互越多的用户，正则化系数越小（更信任本地数据）
    # 交互越少的用户，正则化系数越大（更依赖全局知识）
    interaction_ratio = user_interaction_count / avg_interaction_count
    
    if interaction_ratio >= 1.0:
        # 交互数量高于平均，减少正则化
        reg_coeff = reg_min + (reg_max - reg_min) * math.exp(-interaction_ratio + 1)
    else:
        # 交互数量低于平均，增加正则化
        reg_coeff = reg_min + (reg_max - reg_min) * (2 - interaction_ratio)
    
    # 确保在合理范围内
    reg_coeff = max(reg_min, min(reg_max, reg_coeff))
    return reg_coeff


def get_adaptive_reg_coefficient(config, round_id=0, user_id=0, user_interaction_count=0, 
                                avg_interaction_count=0, performance_history=None):
    """获取自适应正则化系数"""
    if config['adaptive_reg'] == 'none':
        return config['reg']
    elif config['adaptive_reg'] == 'user':
        return adaptive_regularization_user(user_id, user_interaction_count, 
                                          avg_interaction_count, config['reg_min'], config['reg_max'])
    else:
        return config['reg']
