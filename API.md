...
| ------ | ----------------------------- |
| 系统名称   | 产业链风险评估系统                     |
| 基础 URL | `http://8.156.69.49:8080/api` |
...
| `/periods/{industryChainId}` | GET    | 获取产业链下可选数据期间                                                        |
    ...
    "records": [
      {
        ...
        "industryChainId": 1,
        "dataPeriod": "2025-01",
        "filePath": "/datasets/ic/feature_2025-01.xlsx",
        "fileSize": 1024,
...
上传接口（`multipart/form-data`）字段：`file`(文件)、`name`、`typeCode`、`industryChainId`、`dataPeriod`。成功返回 `data: null`。
...
#### GET /api/risk-status/overview/{industryChainId}?dataPeriod=2025-01

该接口对应课题三产业链风险评估结果、产业链完整性评估（三个得分）结果、产业链风险预警结果
